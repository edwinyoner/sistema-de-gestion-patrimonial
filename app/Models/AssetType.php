...
class AssetType extends Model
{
    ...
    // Define una relación de uno a muchos: un tipo de activo puede tener muchos activos.
    public function companyAssets()
    {
        return $this->hasMany(CompanyAsset::class, 'asset_type_id');
    }
    ...
    public static function boot()
    {
        ...
        static::updating(function ($assetType) {
            if ($assetType->isDirty('status') && !$assetType->status && $assetType->companyAssets()->exists()) {
                throw new \Exception('No se puede desactivar este tipo de activo porque hay activos asignados.');
            }
        });
    }
