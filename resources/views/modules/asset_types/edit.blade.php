...
@section('subtitle', 'Editar Tipo de Activo')
@section('content_header_title', 'Tipos de Activos')
@section('content_header_subtitle', 'Editar información del tipo de activo')
...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
            <x-adminlte-card title="Editar Tipo de Activo" theme="warning" icon="fas fa-edit">
                ...
                <form method="POST" action="{{ route('asset_types.update', $assetType->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <x-adminlte-textarea
                        name="description"
                        label="Descripción"
                        placeholder="Actualiza la descripción del tipo de activo"
                        label-class="text-lightblue"
                        rows="3"
                        igroup-size="md">{{ old('description', $assetType->description) }}
                        <x-slot name="prependSlot">
                            <div class="input-group-text bg-warning">
                    ...
                    <div class="form-group">
                        ...
                        <div class="toggle-container">
                            <div class="toggle-wrap">
                                <input type="hidden" name="status" id="status-value" value="{{ $assetType->status ? 1 : 0 }}">
                                <input class="toggle-input" id="holo-toggle" type="checkbox" {{ $assetType->status ? 'checked' : '' }} onchange="document.getElementById('status-value').value = this.checked ? 1 : 0;">
                        ...
                        <small class="form-text text-muted">Desactiva este tipo de activo si no está en uso.</small>
                    </div>

                    <div class="d-flex justify-content-between mt-3">
                        <a href="{{ route('asset_types.index') }}" class="btn btn-sm btn-secondary">
