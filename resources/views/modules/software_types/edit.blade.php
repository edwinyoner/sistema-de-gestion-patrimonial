...
@section('subtitle', 'Editar Tipo de Software')
@section('content_header_title', 'Tipos de Software')
@section('content_header_subtitle', 'Editar información del tipo de software')
...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
            <x-adminlte-card title="Editar Tipo de Software" theme="warning" icon="fas fa-edit">
                ...
                <form method="POST" action="{{ route('software_types.update', $softwareType->id) }}">
                    @csrf
                    @method('PUT')

                    <x-adminlte-input
                        name="name"
                        label="Nombre del Tipo de Software"
                        placeholder="Ej. LICENCIADO"
                        label-class="text-lightblue"
                        value="{{ old('name', $softwareType->name) }}"
                        required
                        id="name">
                    ...
                    <x-adminlte-textarea
                        name="description"
                        label="Descripción"
                        placeholder="Actualiza la descripción del tipo de software"
                        label-class="text-lightblue"
                        rows="3"
                        igroup-size="md">{{ old('description', $softwareType->description) }}
                        <x-slot name="prependSlot">
                            <div class="input-group-text bg-warning">
                    ...
                    <div class="form-group">
                        ...
                        <div class="toggle-container">
                            <div class="toggle-wrap">
                                <input type="hidden" name="status" id="status-value" value="{{ $softwareType->status ? 1 : 0 }}">
                                <input class="toggle-input" id="holo-toggle" type="checkbox" {{ $softwareType->status ? 'checked' : '' }} onchange="document.getElementById('status-value').value = this.checked ? 1 : 0;">
                        ...
                        <small class="form-text text-muted">Desactiva este tipo de software si no está en uso.</small>
                    </div>

                    <div class="d-flex justify-content-between mt-3">
                        <a href="{{ route('software_types.index') }}" class="btn btn-sm btn-secondary">
